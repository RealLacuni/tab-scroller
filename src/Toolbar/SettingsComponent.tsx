  ...
  return (
    <div className="relative">
      <AdjustmentsHorizontalIcon
        className={
          'w-8 h-8 border border-slate-400 cursor-pointer text-slate-600 bg-slate-300 rounded-lg p-0.5 hover:text-white' +
          (isOpen ? ' bg-slate-400' : '')
        }
        onClick={handleClick}
      />
      ...
      {isOpen && (
        <div
          className="absolute top-10 -right-2 flex flex-col justify-between 
         min-h-48 min-w-36  p-2 
        border-2 bg-slate-300 border-slate-400 rounded-sm drop-shadow-lg z-30"
        >
          {/* div take up full height */}
          <h3 className="text-xs text-slate-600 self-center underline pb-1">Settings</h3>
